...
void db_write_txt(DB *db, FILE *stream) {
    ...
    size_t n_int_prop = 0;
    size_t n_str_prop = 0;
    for (size_t i = 0; i < db->properties.count; i++) {
        if (db->properties.at[i].type == PROP_INT) {
            fprintf(stream, "DATATYPE_%zu=%zu\n", i,
                db->properties.at[i].type + (n_int_prop++));
        } else {
            fprintf(stream, "DATATYPE_%zu=%zu\n", i,
                db->properties.at[i].type + (n_str_prop++));
        } 
    }
    ...
    for (size_t i = 0; i < db->properties.count; i++)
        fprintf(stream, "ITEMNAME%zu=" SV_FMT "\n", i,
            SV_FMT_VAL(db->properties.at[i].ITEMNAME));
    ...
    for (size_t i = 0; i < db->properties.count; i++)
        fprintf(stream, "VMEMO_NAME_%zu=" SV_FMT "\n", i,
            SV_FMT_VAL(db->properties.at[i].VMEMO));
    ...
    for (size_t i = 0; i < db->properties.count; i++)
        fprintf(stream, "DEFAULT_VAL_%zu=%d\n", i,
            db->properties.at[i].DEFAULT_VAL);
