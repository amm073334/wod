...
struct Node {
    Node(Token* pos) : pos(pos) {}
    virtual void accept(Visitor* v) = 0;
    Token* pos;
};

struct Stmt : public Node {
    Stmt(Token* pos) : Node(pos) {}
};

struct Expr : public Node {
    Expr(Token* pos) : Node(pos) {}
    WodType type;
    bool assignable;
    bool is_const;
    int32_t const_int;
...
struct FunctionStmt : public Stmt {
    FunctionStmt(Token* pos, WodType return_type, std::string name, std::vector<VarStmt*> params, std::vector<Stmt*> body, bool is_inline)
...
struct BlockStmt : public Stmt {
    BlockStmt(Token* pos, std::vector<Stmt*> stmts)
...
struct ReturnStmt : public Stmt {
    ReturnStmt(Token* pos, Expr* expr)
        : Stmt(pos), expr(expr) {}
...
struct VarStmt : public Stmt {
    VarStmt(Token* pos, bool is_const, WodType type, Expr* arr_len, std::string name, Expr* initializer)
        : Stmt(pos), is_const(is_const), type(type), arr_len(arr_len), name(name), initializer(initializer) {}
...
struct IfStmt : public Stmt {
    IfStmt(Token* pos, Expr* condition, Stmt* then_branch, Stmt* else_branch)
...
struct LoopStmt : public Stmt {
    LoopStmt(Token* pos, Expr* count, Stmt* body)
...
struct CmdStmt : public Stmt {
    CmdStmt(Token* pos, Expr* cmd_id, std::vector<Expr*> int_fields, std::vector<Expr*> str_fields)
...
struct CdbStmt : public Stmt {
    CdbStmt(Token* pos, std::string name, std::vector<VarStmt*> fields)
...
struct AssignExpr : public Expr {
    ...
    AssignExpr(Token* pos, Expr* lhs, AssignOp op, Expr* rhs)
...
struct VariableExpr : public Expr {
    VariableExpr(Token* pos, std::string name)
        : Expr(pos), name(name) {}
...
struct ArrayExpr : public Expr {
    ArrayExpr(Token* pos, std::string name, Expr* index)
        : Expr(pos), name(name), index(index) {}
    void accept(Visitor* v) override { v->visit_ArrayExpr(this); }
    std::string name;
    Expr* index;
    Symbol* sym;
};
...
struct BinaryExpr : public Expr {
    ...
    BinaryExpr(Token* pos, Expr* left, BinaryOp op, Expr* right)
...
struct UnaryExpr : public Expr {
    ...
    UnaryExpr(Token* pos, UnaryOp op, Expr* right)
...
struct CallExpr : public Expr {
    CallExpr(Token* pos, std::string name, std::vector<Expr*> args)
...
struct IntLiteralExpr : public Expr {
    IntLiteralExpr(Token* pos, int32_t value)
...
struct StrLiteralExpr : public Expr {
    StrLiteralExpr(Token* pos, std::string value)
        : Expr(pos), value(value) {}
...
struct FStringExpr : public Expr {
    ...
    FStringExpr(Token* pos, std::vector<Fragment> frags)
